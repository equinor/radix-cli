...
// New creates a new buildstatus API client with basic auth credentials.
...
// New creates a new buildstatus API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) GetBuildStatus(params *GetBuildStatusParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetBuildStatusOK, error) {
	...
	op := &runtime.ClientOperation{
		ID:                 "getBuildStatus",
		Method:             "GET",
		PathPattern:        "/applications/{appName}/environments/{envName}/buildstatus",
	...
	msg := fmt.Sprintf("unexpected success response for getBuildStatus: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
